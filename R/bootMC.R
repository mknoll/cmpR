bootMC <- function(obj, B=1000, typeRes="HC3", typeEps="Liu1988", trunc=1000) {
    frm <- obj@formula
    data <- obj@data
	...
	#if (orig) {
	#    probab <- exp(posterior(proposal, data[,obj@covar], origD[,obj@depVar]) - posterior(chain[i,], data[,obj@covar], origD[,obj@depVar]))
	#} else {
